...
def test_train_lightgbm_writes_standard_artifacts(tmp_path, monkeypatch):
    pytest.importorskip("lightgbm")

    import src.train as train_mod

    # Synthetic tabular dataset (small and deterministic), no real files touched.
    n, h = 60, 3
    X = pd.DataFrame(
        {
    ...
    meta = pd.DataFrame({"symbol": ["SYN"] * n, "source_file": ["SYN.xlsx"] * n, "origin_ds": pd.date_range("2024-01-01", periods=n)})
    ...
    pred_path = run_dir / "predictions_val_lightgbm.npy"
    ...
    assert metrics["lightgbm"]["corr_basis"] == "delta"
    assert "corr_by_horizon" in metrics["lightgbm"]
